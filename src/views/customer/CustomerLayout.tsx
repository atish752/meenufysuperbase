...
export default function CustomerLayout({ tableId }: Props) {
  ...
  const hasActiveRestaurant = !!(new URLSearchParams(window.location.search).get('restaurant') || state.activeCustomerRestaurantId);
  const rId = new URLSearchParams(window.location.search).get('restaurant') || getActiveRestaurantId(state);
  const restaurant = getActiveRestaurantInfo(state, rId);
  const isClosed = hasActiveRestaurant && (isRestaurantClosed(restaurant?.openTime, restaurant?.closeTime, restaurant?.daySpecificHours) || restaurant?.isManualClosed === true) && activeOrders.length === 0;
